...
const t = require("@lightscript/babel-preset").api.types;
...
function analyzeScope(ast, parserOptions, overrideScopeMgr, overrideReferencer) {
  const options = {
    ignoreEval: true,
    optimistic: false,
    directive: false,
  ...
  if (OriginalReferencer._babelEslintPatched) {
    throw new Error("`@lightscript/eslint` is incompatible with `babel-eslint`. Somehow they have both been loaded into the same instance of `eslint`.")
  }

  options.childVisitorKeys = childVisitorKeys;
  overrideReferencer = overrideReferencer || Referencer;
  overrideScopeMgr = overrideScopeMgr || escope.ScopeManager;

  const scopeManager = new overrideScopeMgr(options);
  const referencer = new overrideReferencer(options, scopeManager);
...
// Allow referencer override
analyzeScope.Referencer = Referencer;
